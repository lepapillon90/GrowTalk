...
### ✅ 성능 및 최적화
- **가상 스크롤링**: `react-window`를 활용한 대용량 메시지 렌더링 최적화 (`VirtualMessageList`)
- **메시지 페이지네이션**: Firestore `limit` 및 `startAfter` 커서 기반 로딩
- **실시간 리스너 최적화**: 필요한 데이터만 구독하여 읽기 비용 절감
- **이미지 최적화**: 클라이언트 측 압축/업로드 및 지연 로딩 (`loading="lazy"`)
- **번들 분석**: `@next/bundle-analyzer` 도입 및 최적화 환경 구축

## 3. 단기 기술 목표 (2026년 1분기)
### 🚀 성능 최적화 (진행 중)
- [x] **가상 스크롤링**: `react-window` 구현 완료
- [x] **이미지 지연 로딩 개선**: 네이티브 `loading="lazy"` 속성 확인 및 적용 완료
- [x] **번들 크기 분석**: `@next/bundle-analyzer` 설치 및 분석 스크립트 추가 완료
