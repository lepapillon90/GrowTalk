...
| **Firestore (Schedule)** | 일정 관리 | ✅ 구현 완료 (`users/{id}/schedules`) |
...
**일정(Schedule) 구조:**
```
users/{userId}/schedules/{scheduleId} (Sub-collection)
```
*   개인별 일정은 `users` 하위에 두어 보안 규칙(`request.auth.uid == userId`) 설정이 용이하며, 쿼리 범위가 개인으로 한정되어 효율적입니다.
...
- [x] 클라이언트 측 이미지 압축 도입
- [x] **메시지 페이지네이션 (`limit`) 적용** (최우선 순위)
- [x] **실시간 리스너 최적화** (최우선 순위)
- [x] **메시지 삭제** (최우선 순위)
- [x] **메시지 수정** (최우선 순위)
- [x] **메시지 읽음 표시** (최우선 순위)
